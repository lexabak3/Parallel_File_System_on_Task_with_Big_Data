...
print(cluster_spec)
print(cluster, my_job_name, my_task_index)
print()

server = tf.distribute.Server(server_or_cluster_def=cluster_spec, job_name=my_job_name, task_index=my_task_index)
...
def conv2d(in_x, in_w):
    return tf.nn.conv2d(in_x, in_w, strides=[1, 1, 1, 1], padding='VALID')


def max_pool_2x2(in_x):
    return tf.nn.max_pool(in_x, ksize=[1, 2, 2, 1],
                          strides=[1, 2, 2, 1], padding='SAME')


with tf.device('/job:worker/task:{}'.format(my_task_index)):
    x = tf.compat.v1.placeholder(tf.float32, shape=(None, 3072))
    y = tf.compat.v1.placeholder(tf.uint8, shape=(None, 1))
